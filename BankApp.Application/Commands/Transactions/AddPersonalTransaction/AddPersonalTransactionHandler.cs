...
{
	public class AddPersonalTransactionHandler : IRequestHandler<AddPersonalTransactionCommand, CommandsStatus>
	{
        ...
        public AddPersonalTransactionHandler(BankAppDbContext context,
        ...
        public async Task<CommandsStatus> Handle(AddPersonalTransactionCommand request, CancellationToken cancellationToken)
        {
            ...
            using (var transaction = await context.Database.BeginTransactionAsync())
            {
                try
                {
                    context.Entry(sourceWallet.Currency).State = EntityState.Detached;
                    context.Entry(destinationWallet.Currency).State = EntityState.Detached;

                    await context.Transactions.AddAsync(newTransaction, cancellationToken);

                    context.Entry(sourceWallet).State = EntityState.Modified;
                    context.Entry(destinationWallet).State = EntityState.Modified;
